...
public class Matrix {
	...
	public boolean equals(Object other)	{
		...
		// First test dimensions of the matrices in question to see if they are equal
		if(this.numRows != matrix.numRows || this.numColumns != matrix.numColumns) {
			return false;
		}
		// If dimensions are equal then check the elements
		...
		for(int i = 0; i < this.numRows; i++) {
			for(int j = 0; j < matrix.numColumns; j++) {
				...
				for(int k = 0; k < matrix.numRows; k++) {
					newMatrixData += this.data[i][k] * matrix.data[k][j];	
				}
				newMatrix.data[i][j] = newMatrixData;
			}
		}
		return newMatrix;
	}
	...
	public Matrix plus(Matrix matrix) {
		...
		Matrix newMatrix = new Matrix(new int[matrix.numRows][matrix.numColumns]);
		for(int i = 0; i < matrix.numRows; i++) {
			for(int j = 0; j < matrix.numColumns; j++) {
